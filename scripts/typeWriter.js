    ...
    for (a in list) {
        ...
        if (arguments[1] === "true") {
            clear = setInterval(function () { // Animation start
                ...
                if(text[end]==="<" && end <text.length - 30){
                  end = end+2;
                }
                ...
                style.sheet.insertRule("@-webkit-keyframes cursor {0% { opacity : 0.8;}100% { opacity : 0.5;}}", 0);

                style.sheet.insertRule("@keyframes cursor {0% { opacity : 0.8;}100% { opacity : 0.5;}}", 0);
